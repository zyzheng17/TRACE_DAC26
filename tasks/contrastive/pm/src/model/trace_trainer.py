...
class TRACETrainer(pl.LightningModule):
    """Lightning trainer for PM netlist contrastive TRACE."""
    ...
    def forward_cls(self, batch):
        ...
        orig_h = self.encoder(batch.pm_x, batch.pm_edge_index, batch.pm_forward_level, batch.pm_forward_index)
        syn_h = self.encoder(batch.syn_x, batch.syn_edge_index, batch.syn_forward_level, batch.syn_forward_index)
        orig_h = orig_h[torch.logical_and(batch.pm_forward_level != 0, batch.pm_backward_level == 0)]
        syn_h = syn_h[torch.logical_and(batch.syn_forward_level != 0, batch.syn_backward_level == 0)]
