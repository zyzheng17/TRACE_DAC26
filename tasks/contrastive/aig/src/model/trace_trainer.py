...
class TRACETrainer(pl.LightningModule):
    """Lightning trainer for AIG contrastive TRACE."""
    ...
    def forward_cls(self, batch):
        ...
        orig_h = self.encoder(batch.gate, batch.edge_index, batch.forward_level, batch.forward_index)
        syn_h = self.encoder(batch.syn_gate, batch.syn_edge_index, batch.syn_forward_level, batch.syn_forward_index)
        orig_h = orig_h[torch.logical_and(batch.forward_level != 0, batch.backward_level == 0)]
