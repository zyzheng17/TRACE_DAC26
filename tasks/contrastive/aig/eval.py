...
def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate TRACE on AIG contrastive retrieval.')
    parser.add_argument('--data_path', type=Path, default=Path('../../../data/aig/forgeeda_pm_aig_test.pt'))
    parser.add_argument('--checkpoint', type=Path, default=Path('../../../checkpoints/trace_contrastive_aig_rec.ckpt'))
...
def load_encoder_checkpoint(model, checkpoint_path):
    ...
    for key, value in state.items():
        if key.startswith('encoder.'):
            mapped[key] = value
        elif key.startswith('aig_encoder.'):
            mapped[key.replace('aig_encoder.', 'encoder.', 1)] = value
