...
def top_sort(edge_index, graph_size):
    # 添加输入验证
    if edge_index.dim() != 2:
        raise ValueError(f"edge_index必须是2维张量，但得到的是{edge_index.dim()}维张量，形状为{edge_index.shape}")
    
    if edge_index.size(0) != 2:
        raise ValueError(f"edge_index的第一维必须是2，但得到的是{edge_index.size(0)}")
    
    if edge_index.size(1) == 0:
        # 如果没有边，返回所有节点为同一层级的顺序
        return torch.zeros(graph_size, dtype=torch.long)
    ...
    while unevaluated_nodes.any():
        ...
        # 创建一个布尔掩码，标记哪些边的父节点还没有被评估
        unevaluated_mask = numpy.array([unevaluated_nodes[parent] for parent in parent_nodes], dtype=bool)
...
def return_order_info(edge_index, num_nodes):
    ...
    if edge_index.size(1) > 0:
        ...
        # 如果没有边，反向层级也是全零
        backward_level = torch.zeros(num_nodes, dtype=torch.long)
